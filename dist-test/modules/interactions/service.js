...
exports.rebuildProjection = rebuildProjection;
const database_1 = __importDefault(require("../../config/database"));
const errors_1 = require("./errors");
function formatEventResponse(event, metadata) {
    ...
    if (event.type === 'SCHEDULE') {
        if (meta?.scheduledDate)
            base.scheduledDate = meta.scheduledDate;
        if (meta?.appointmentId)
            base.appointmentId = meta.appointmentId;
    }
    return base;
}
function formatDuplicateResponse(event) {
    return {
        interactionId: event.id,
        type: event.type,
        isDuplicate: true,
...
async function ensurePropertyExists(propertyId) {
    ...
    if (!property) {
        throw new errors_1.InteractionApiError('PROPERTY_NOT_FOUND', 'Property not found', 404);
    }
}
async function bootstrapProjection(tx, userId, propertyId) {
    await tx.$executeRaw `
...
async function findDuplicateEvent(userId, propertyId, type, idempotencyKey) {
    return database_1.default.userInteractionEvent.findUnique({
        where: {
...
async function insertEvent(tx, userId, input, type) {
...
async function recordSimpleEvent(userId, input, type, successMessage) {
    await ensurePropertyExists(input.propertyId);
    const existing = await findDuplicateEvent(userId, input.propertyId, type, input.idempotencyKey);
    if (existing) {
        return {
            statusCode: 200,
            body: {
    ...
    const event = await database_1.default.userInteractionEvent.create({
        data: {
            userId,
            propertyId: input.propertyId,
            type,
...
async function recordToggleEvent(userId, input, options) {
    await ensurePropertyExists(input.propertyId);
    const eventType = options.isAdd ? options.addType : options.removeType;
    const existing = await findDuplicateEvent(userId, input.propertyId, eventType, input.idempotencyKey);
    if (existing) {
        return {
            statusCode: 200,
            body: {
    ...
    try {
        ...
        if (error instanceof errors_1.InteractionApiError) {
            throw error;
        }
        throw error;
    }
...
async function rebuildProjection(userId, propertyId) {
    const events = await database_1.default.userInteractionEvent.findMany({
        where: {
            userId,
            propertyId,
    ...
    let isLiked = false;
    let isSaved = false;
    let lastLikeEventId = null;
    let lastSaveEventId = null;
    ...
    return database_1.default.userPropertyState.upsert({
...
function computeLikeCycles(events) {
    const likeEvents = events.filter((e) => ['LIKE_ADDED', 'LIKE_REMOVED'].includes(e.type));
    const cycles = [];
    let openLike = null;
...
function extractLocalizedText(value, lang = 'en') {
...
function formatHistoryProperty(property) {
    ...
    const categoryObj = property.category;
...
class InteractionService {
    recordView(userId, input) {
        return recordSimpleEvent(userId, input, 'VIEW', 'View recorded');
    }
    likeProperty(userId, input) {
        return recordToggleEvent(userId, input, {
            addType: 'LIKE_ADDED',
            removeType: 'LIKE_REMOVED',
            addMessage: 'Property liked',
            removeMessage: 'Property unliked',
            alreadyError: new errors_1.InteractionApiError('ALREADY_LIKED', 'This property is already liked.', 409),
            notError: new errors_1.InteractionApiError('NOT_LIKED', 'This property is not currently liked.', 409),
            projectionField: 'isLiked',
            lastEventField: 'lastLikeEventId',
            isAdd: true,
        });
    }
    unlikeProperty(userId, input) {
        return recordToggleEvent(userId, input, {
            addType: 'LIKE_ADDED',
            removeType: 'LIKE_REMOVED',
            addMessage: 'Property liked',
    ...
    saveProperty(userId, input) {
        return recordToggleEvent(userId, input, {
            addType: 'SAVE_ADDED',
            removeType: 'SAVE_REMOVED',
            addMessage: 'Property saved',
            removeMessage: 'Property removed from saved',
            alreadyError: new errors_1.InteractionApiError('ALREADY_SAVED', 'This property is already saved.', 409),
            notError: new errors_1.InteractionApiError('NOT_SAVED', 'This property is not currently saved.', 409),
            projectionField: 'isSaved',
            lastEventField: 'lastSaveEventId',
            isAdd: true,
        });
    }
    unsaveProperty(userId, input) {
        return recordToggleEvent(userId, input, {
            addType: 'SAVE_ADDED',
            removeType: 'SAVE_REMOVED',
            addMessage: 'Property saved',
    ...
    async getHistory(userId, query) {
        ...
        const [total, events] = await Promise.all([
            database_1.default.userInteractionEvent.count({ where }),
    ...
    async exportUserEvents(userId, after) {
        const where = { userId };
        if (after) {
        ...
        const [events, likedStates, savedStates] = await Promise.all([
            ...
            database_1.default.userPropertyState.findMany({
    ...
    validateSource(source) {
        if (source === undefined)
            return undefined;
        if (!errors_1.INTERACTION_SOURCE_VALUES.includes(source)) {
            throw new errors_1.InteractionApiError('INVALID_SOURCE', 'Source must be a valid InteractionSource enum value', 400, {
                providedValue: source,
                allowedValues: [...errors_1.INTERACTION_SOURCE_VALUES],
            });
        }
        return source;
    }
    /** Daily-deduped view for property detail pages; returns true when a new view was recorded. */
    async trackLegacyDailyView(propertyId, userId) {
    ...
    async getSavedAt(userId, propertyId) {
        const state = await database_1.default.userPropertyState.findUnique({
            where: { userId_propertyId: { userId, propertyId } },
        });
        if (!state?.isSaved)
            return null;
        if (state.lastSaveEventId) {
            const event = await database_1.default.userInteractionEvent.findUnique({
    ...
    async listSavedPropertyRecords(userId) {
        return database_1.default.userPropertyState.findMany({
            where: {
                userId,
                isSaved: true,
