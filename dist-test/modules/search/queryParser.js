...
exports.ALLOWED_AMENITIES = void 0;
exports.computeConfidence = computeConfidence;
exports.sanitizeParsedFilters = sanitizeParsedFilters;
exports.parseQueryLocally = parseQueryLocally;
exports.mergeParsedFilters = mergeParsedFilters;
const propertyTypes_1 = require("./propertyTypes");
const pricePhrases_1 = require("./pricePhrases");
...
const GENERIC_HOUSE_PHRASE = /\b(?:need|want|looking for|searching for|find|get)\s+(?:a\s+)?(?:the\s+)?house\b/i;
...
const PROPERTY_TYPE_SET = new Set([
    'apartment',
    'villa',
    'studio',
    'house',
...
function applyPriceRulesFromText(query, filters) {
    const lower = query.toLowerCase();
    const bounds = (0, pricePhrases_1.extractPriceBounds)(query);
...
function parsePropertyTypeFromQuery(query) {
    if (GENERIC_HOUSE_PHRASE.test(query)) {
        return null;
    }
    for (const { pattern, type } of propertyTypes_1.PROPERTY_TYPE_PARSE_ORDER) {
...
function computeConfidence(filters, query) {
    let score = 0;
    if (filters.location)
        score += 0.3;
...
function sanitizeParsedFilters(raw, query, displayCurrency = 'ETB') {
    const amenities = Array.isArray(raw.amenities)
        ? raw.amenities
            .map((a) => String(a).trim().toLowerCase())
            .filter((a) => exports.ALLOWED_AMENITIES.includes(a))
        : [];
...
function parseQueryLocally(query, displayCurrency = 'ETB') {
    const q = query.trim();
    ...
    const filters = {
        ...
        minPrice: null,
        maxPrice: null,
        priceCurrency: (0, pricePhrases_1.detectPriceCurrency)(q),
        currency: displayCurrency,
        amenities: [],
        propertyType: null,
        keywords: [],
...
function mergeParsedFilters(primary, fallback, query, displayCurrency = 'ETB') {
