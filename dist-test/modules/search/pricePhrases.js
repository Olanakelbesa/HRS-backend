...
exports.detectPriceCurrency = detectPriceCurrency;
exports.extractPriceBounds = extractPriceBounds;
function parseNumberToken(raw) {
    const value = Number(raw.replace(/,/g, ''));
    return Number.isFinite(value) && value >= 0 ? value : null;
}
function detectPriceCurrency(query) {
    if (/\$|usd\b|dollars?\b/i.test(query))
        return 'USD';
    if (/\bbirr\b|\betb\b|\bbr\b/i.test(query))
        return 'ETB';
    return 'ETB';
}
/**
...
function extractPriceBounds(query) {
    ...
    const between = query.match(/\bbetween\s*([\d,]+)\s*(?:birr|etb|br|usd|\$)?\s*(?:and|to|-)\s*([\d,]+)\s*(?:birr|etb|br|usd)?/i);
    if (between) {
