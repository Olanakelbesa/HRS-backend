...
exports.hasMeaningfulPreferences = hasMeaningfulPreferences;
exports.scorePropertyAgainstPreferences = scorePropertyAgainstPreferences;
...
function extractPriceEtb(price) {
    const p = price;
    if (!p)
        return 0;
    if (p.amountEtb != null && Number.isFinite(p.amountEtb))
        return p.amountEtb;
    const value = Number(p.value ?? 0);
    const currency = String(p.currency ?? 'ETB').toUpperCase();
    return currency === 'USD' ? value * 56 : value;
}
function extractCategoryType(category) {
...
function extractLocationStrings(property) {
    ...
    if (loc?.city)
        parts.push(String(loc.city).toLowerCase());
    if (loc?.neighborhood)
        parts.push(String(loc.neighborhood).toLowerCase());
    if (loc?.subcity)
        parts.push(String(loc.subcity).toLowerCase());
    if (addr?.en)
        parts.push(String(addr.en).toLowerCase());
    if (addr?.am)
        parts.push(String(addr.am).toLowerCase());
    return parts;
}
function extractAmenities(amenities) {
    ...
    return amenities.map((entry) => {
        ...
        if (entry && typeof entry === 'object' && 'en' in entry) {
            return String(entry.en ?? '').toLowerCase();
        }
        return String(entry).toLowerCase();
    });
}
function hasMeaningfulPreferences(pref) {
    const locations = pref.preferredLocations ?? [];
    return (pref.preferredPriceMin != null ||
...
function scorePropertyAgainstPreferences(property, pref) {
    let score = 0;
    ...
    for (const loc of preferredLocs) {
        ...
        const addressMatch = locStrings.some((part) => part.includes(addr) || addr.includes(part));
        if (addressMatch) {
            score += 55;
            break;
        }
